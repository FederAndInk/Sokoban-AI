...
public class ExempleJavaFXAnime extends Application {
	int x, y;
	Canvas can;
	Image img;
	Random r;
	...
	public void start(Stage primaryStage) {
		...
		can.setOnMouseClicked(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent e) {
				...
				if (toucheBonhomme(e.getX(), e.getY())) {
					i++;
			...
			public void handle(long now) {
				if (animationEnCours) {
					...
					if (progres > 1) {
						...
						x = (int) Math.round((1 - prop) * srcX + prop * dstX);
						y = (int) Math.round((1 - prop) * srcY + prop * dstY);
					}
					trace();
				}
			}
	...
	// Déplace l'image aléatoirement en initiant une animation
	void deplacementAleatoire() {
		if (!animationEnCours) {
			srcX = x;
			srcY = y;
			dstX = r.nextInt((int) can.getWidth());
			dstY = r.nextInt((int) can.getHeight());
